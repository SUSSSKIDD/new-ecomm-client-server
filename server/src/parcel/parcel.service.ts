...
export class ParcelService {
  ...
  async approveParcel(id: string, dto: ApproveParcelDto) {
    ...
    if (parcel.status !== ParcelStatus.PENDING) {
      throw new BadRequestException(
        `Cannot approve parcel in status ${parcel.status}`,
      );
    }
    ...
    this.logger.log(
      `Parcel ${parcel.parcelNumber} APPROVED with COD ${dto.codAmount}`,
    );
    return updated;
  }
  ...
  private static readonly VALID_PARCEL_TRANSITIONS: Record<
    string,
    ParcelStatus[]
  > = {
  ...
  async updateStatus(id: string, dto: UpdateParcelStatusDto) {
    ...
    if (dto.status === ParcelStatus.DELIVERED) {
      throw new BadRequestException(
        'DELIVERED status can only be set by rider',
      );
    }
  ...
  private async cancelParcel(
    ...
    parcel: {
      parcelNumber: string;
      assignment?: { id: string; deliveryPersonId: string } | null;
    },
  ) {
    ...
    this.orderPoolService
      ...
      .catch((err) =>
        this.logger.error(
          `Failed to remove cancelled parcel ${id} from pool: ${err.message}`,
        ),
      );
    ...
    this.logger.log(
      `Parcel ${parcel.parcelNumber} CANCELLED — pool + riders cleaned up`,
    );
    return cancelled;
  }
  ...
  async triggerDeliveryAssignment(id: string) {
    ...
    if (
      parcel.status !== ParcelStatus.READY_FOR_PICKUP &&
      parcel.status !== ParcelStatus.ASSIGNED
    ) {
      throw new BadRequestException(
        'Parcel must be READY_FOR_PICKUP to assign',
      );
    }
    ...
    return {
      message: 'Parcel assignment sequence started. Finding nearby riders...',
    };
  }
}
