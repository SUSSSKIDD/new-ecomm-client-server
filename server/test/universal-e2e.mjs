...
const STORE_TYPES = ['GROCERY', 'PIZZA_TOWN', 'FASHION', 'HOME_SERVICE', 'DROP_IN_FACTORY'];
const CATEGORIES_PER_TYPE = {
  GROCERY: 'Atta, Rice & Dal',
  PIZZA_TOWN: 'Pizza',
  FASHION: 'T-Shirts',
  HOME_SERVICE: 'General Repair',
...
await step('Super admin login', async () => {
  const r = await api.post('/auth/super-admin/login', { phone: '+917785945524', pin: '5015' });
...
await step('Invalid admin PIN rejected', async () => {
  const r = await api.post('/auth/super-admin/login', { phone: '+917785945524', pin: '9999' });
...
for (const storeType of STORE_TYPES) {
  ...
  if (storeType === 'FASHION' || storeType === 'HOME_SERVICE') {
    await step(`Create custom subcategory for ${storeType}`, async () => {
      const r = await api.post('/stores/subcategories/custom/admin', {
        storeType,
        name: CATEGORIES_PER_TYPE[storeType]
      }, auth(adminToken));
      assert(r.status === 201 || r.status === 200, `${r.status}: ${r.data?.message}`);
    });
  }
...
await step('Products hidden far away', async () => {
  ...
  assert(r.status === 200, `${r.status}`);
  const list = r.data.data;
  assert(list.length === 0, `Expected 0 products far away, got ${list.length}`);
});
